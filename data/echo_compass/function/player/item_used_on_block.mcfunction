...
execute as @a if predicate echo_compass:offhand if data entity @s Inventory[{Slot:-106b}].components."minecraft:lodestone_tracker" unless data entity @s Inventory[{Slot:-106b}].components."minecraft:consumable" at @s run return run function echo_compass:link/offhand